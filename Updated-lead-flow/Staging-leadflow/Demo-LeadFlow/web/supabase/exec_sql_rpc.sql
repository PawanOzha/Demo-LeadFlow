...
--
-- Parameter binding: use #>> ARRAY['n'] for jsonb *arrays* (PostgREST / JSON).
-- query_params->>0 treats 0 as a *text object key*, not an array index, so bound
-- $1..$N were often NULL → invalid_parameter_value → old code swallowed the error
-- and returned zero rows.
--
-- Wrapping: concatenate instead of format('%s', query_text) so literal % in SQL
-- (e.g. ILIKE patterns) cannot corrupt the wrapped statement.
