...
export async function fetchAtlRoutingTimelines(
  ...
  for (let i = 0; i < leadIds.length; i += SQL_RPC_SAFE_PAGE_SIZE) {
    const chunk = leadIds.slice(i, i + SQL_RPC_SAFE_PAGE_SIZE);
    const handoffRows = await dbQuery<{
      ...
      `SELECT "leadId" AS lead_id, "createdAt" AS created_at, action
       FROM "LeadHandoffLog"
       WHERE "leadId" = ANY(${rpcJsonParamTextArrayUnpack(1)})
         AND action = ANY(${rpcJsonParamTextArrayUnpack(2)})
