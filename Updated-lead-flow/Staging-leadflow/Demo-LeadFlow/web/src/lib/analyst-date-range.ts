...
export async function analystRangeParams(
  searchParams:
    ...
    | Record<string, string | string[] | undefined>,
  opts?: {
    /** When true, ignore partial ranges and require both `from` + `to`. */
    requireCompleteDateRange?: boolean;
  },
  ...
  const requireCompleteDateRange = Boolean(opts?.requireCompleteDateRange);
  const hasCompleteDateRange = Boolean(fromRaw && toRaw);
  const { from, to } =
    requireCompleteDateRange && !hasCompleteDateRange
      ? { from: null, to: null }
      : canonicalizePortalDatePair(fromRaw, toRaw);
  return {
    from,
    to,
