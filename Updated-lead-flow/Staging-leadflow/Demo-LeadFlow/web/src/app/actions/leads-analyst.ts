...
export async function createLeadAnalyst(formData: FormData) {
  ...
  const duplicate = await dbQueryOne<{ id: string }>(
    `SELECT id
     FROM "Lead"
     WHERE "createdById" = $1
       AND (
         (
           $2::text IS NOT NULL
           AND NULLIF(regexp_replace(COALESCE(phone, ''), '\\D', '', 'g'), '') =
               NULLIF(regexp_replace($2::text, '\\D', '', 'g'), '')
         )
         OR ($3::text IS NOT NULL AND lower(COALESCE("leadEmail", '')) = lower($3::text))
       )
     LIMIT 1`,
    [session.id, phone || null, leadEmail],
  );
  if (duplicate) {
    return {
      error:
        "A lead with the same phone or email already exists in your list.",
    };
  }
