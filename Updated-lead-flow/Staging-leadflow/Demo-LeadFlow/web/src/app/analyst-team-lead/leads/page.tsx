...
export default async function AnalystTeamLeadLeadsPage({
  ...
  const [countRows, pagedLeadRows, exportLeadRows] = await timedServerBlock(
  ...
  const hasServerFilters = Boolean(
    statusFilter || analystIdFilter || sourceFilter || q,
  );
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6">
      <div className="grid gap-6 lg:grid-cols-[280px_minmax(0,1fr)]">
        <aside className="space-y-3 lg:sticky lg:top-2 lg:self-start lg:max-h-[calc(100dvh-1rem)] lg:overflow-y-auto">
          <AtlLeadsFiltersBar
            status={statusFilter}
        ...
        <div className="space-y-6 min-w-0">
          ...
          <AtlAllLeadsTableClient
            key={`${safePage}|${perPage}|${from ?? ""}|${to ?? ""}|${statusFilter ?? ""}|${analystIdFilter ?? ""}|${sourceFilter ?? ""}`}
